...
export function SiteFooter() {
  return (
    <footer className="bg-surface-ink text-surface-white rounded-t-[30px] pt-24 md:rounded-t-[50px] lg:pt-31.25">
            ...
            >
              {FOOTER_COLUMNS.map((column) => (
                <div key={column.title}>
                  <h2 className="text-lg leading-none text-[#A6A6A6] lg:text-xl">
                    {column.title}
                  </h2>
                  ...
                  <ul className="mt-4 flex flex-col gap-1">
                    {column.links.map((link) => (
                      <li key={link.label}>
                        <Link
                          href={link.href}
                          className="hover:text-surface-white text-xs leading-[1.6] text-white transition-colors"
                        >
                          {link.label}
                        </Link>
                      </li>
