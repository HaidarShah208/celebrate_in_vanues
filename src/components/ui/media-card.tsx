...
export function MediaCard({ title, image, badge }: MediaCardProps) {
  return (
    <article className="group relative aspect-[3/4] shrink-0 basis-[78%] snap-start overflow-hidden rounded-2xl sm:basis-[46%] lg:basis-[calc((100%-4.5rem)/4)]">
      ...
      <div className="from-surface-ink/85 via-surface-ink/25 absolute inset-0 bg-gradient-to-t to-transparent" />
      ...
      <h3 className="text-surface-white absolute inset-x-6 bottom-6 pr-6 text-[22px] leading-[1.25] font-semibold sm:text-[26px] lg:text-[28px]">
        {title}
      </h3>
    </article>
  );
